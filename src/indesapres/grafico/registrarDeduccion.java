...
public class registrarDeduccion extends javax.swing.JFrame {
    ...
    public Deducciones enviarDatos() {
        ...
        float prestamo = Float.parseFloat(String.valueOf(tm.getValueAt(0, 4)));
        ...
        ded = new Deducciones(idDeduccion, Fecha, idPrestamo, nombre, prestamo, deduccion, Saldo, contador);
        return ded;
    }
    ...
    public void obtenerSaldo() {
        String idd = jidDeduccion.getText();
        String idPres = jidPrestamo.getText();
        ServiciosDB service = new ServiciosDB();
        Prestamos pres = service.findByIdPrestamos(idPres);
        String[] obnum = idd.split("\\-");
        String num = obnum[2];
        int numero = Integer.parseInt(num);
        int numant = numero - 1;
        String numante = Integer.toString(numant);
        int plazo = (int) pres.getPlazo() * 2;
        String id = obnum[0] + "-" + obnum[1] + "-" + numante + "-" + "de" + plazo;
        if ("".equals(id)) {
    ...
    public void setearSaldo(Deducciones dedu) {
        float deduccion = dedu.getDeduccion();
        float Saldo = dedu.getSaldoDeudor();
        float SaldoDeudor = Saldo - deduccion;
        jTable2.setValueAt(SaldoDeudor, 0, 8);
    }
    ...
    public void setearPrestamo(Prestamos pres) {
        ...
        float Saldo;
        if ("1".equals(numero)) {
            Saldo = pres.getCapitalinteres() - pres.getDeduccion();
